/*
  # Professional Email Service with Resend Integration
  
  Handles all email notifications for TrustInvoice with professional templates,
  plan-based restrictions, tracking, and comprehensive error handling.
*/
...
async function generateEmailContent(request: EmailRequest) {
  ...
  return {
    ...
    attachments: invoiceData.pdfUrl ? [{
...
function getBaseTemplate(plan: string): string {
  ...
  const logoUrl = 'https://images.unsplash.com/photo-1560472354-b33ff0c44a43?w=100&h=100&fit=crop&crop=center';
...
function generateInvoiceCreatedContent(data: any): string {
  return `
    ...
    <p style="color: #64748b; font-size: 14px; margin-top: 20px;">
      Please review the invoice details and make payment by the due date to avoid any late fees.
    </p>
  `;
}
...
function generateContractDeployedContent(data: any): string {
  return `
    ...
    <p style="font-size: 16px; margin-bottom: 20px;">
      Your invoice <strong>${data.invoiceNumber}</strong> has been secured with a smart contract on the blockchain.
    </p>
